...
const UserManagement = () => {
  ...
  const fetchData = useCallback(async (page = 1, query = '') => {
    try {
      if (page === 1 && !stats) setLoading(true);

      const [statsRes, usersRes] = await Promise.all([
        stats ? Promise.resolve({ data: stats }) : adminService.getUserStats(),
        adminService.getUsers(page, 15, query),
      ]);
      ...
      setPagination({
        ...
        totalPages: usersRes.data.total_pages
      });
